...
import com.imooc.web.dto.ResponseDTO;
...
/**
    ...
    public ResponseDTO minioUpload(MultipartFile file, String fileName, String bucketName) {
        try {
...
/*} else {
            ...
            if (e.getMessage().contains("ORA")) {
                return ResponseDTO.error("上传失败:【查询参数错误】");
            }
...
/**
    ...
    public ResponseDTO downloadFile(String objectName, String bucketName, HttpServletResponse response) {
        try {
            MinioClient minioClient = new MinioClient(ip, accessKey, secretKey);
            ...
            return ResponseDTO.ok(objectName + "下载成功");
        } catch (Exception e) {
            e.printStackTrace();
            if (e.getMessage().contains("ORA")) {
                return ResponseDTO.error("下载失败:【查询参数错误】");
            }
            return ResponseDTO.error("下载失败：【" + e.getMessage() + "】");
        }
    }
